...
function SortOptionsBar() {
  ...
  return (
    <div className="sort-options-bar d-flex flex-wrap justify-content-end align-items-center gap-2 mb-3 p-2">
      ...
      <ButtonGroup size="sm">
        <Button
          className={isSortActive('createdAt', 'desc') ? 'btn-custom-primary' : 'btn-custom-outline-primary'}
          onClick={() => handleSortChange('createdAt', 'desc')}
        >
          <ClockHistory className="me-1" /> {t('sortOptionsBar.newest', 'Mới nhất')}
        </Button>
        <Button variant="outline-primaryy" disabled title={t('sortOptionsBar.comingSoon', 'Sắp có')}> 🔥 {t('sortOptionsBar.bestSelling', 'Bán chạy')} </Button>
      </ButtonGroup>
      <DropdownButton
